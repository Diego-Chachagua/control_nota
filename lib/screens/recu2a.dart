...
class Recupera2A extends StatefulWidget {
  String anio;
  String seccion;
  var materia1;

  Recupera2A(this.anio, this.seccion, this.materia1, {super.key});

  @override
  State<Recupera2A> createState() => _Recupera2AState();
}

class _Recupera2AState extends State<Recupera2A> {
    ...
    (() async {
      ...
      if (reslt != "noExisten") {
        for (var i = 0; i < reslt.length; i++) {
          ...
          if(promediop>5){
          double calculo=promediop*0.85;//multiplicar nota por el 85%
            double  ava=av*0.15;//multiplicar nota avanzo por el 15%
            promedioca=calculo+ava;//sumar notas
          }else if(promediop<=5 ){//si el promedio  es menor  a la nota de 6 y la re1 es distinta de 0,
            if(r1!="0" || r1!=0){//si la re1 es diferente de cero se realizan nuevos calculos 
            double s1=(promediop+r1)/2;//se calcula nuevo promedio con la re1
            //calculo de nuevo promedio con re1
            double  calculo=s1*0.85;
            var ava=av*0.15;
            promedioca=calculo+ava;//suma del nuevo promedio con la avanzo
            }  else{//si no re1 esta vacio 0 igual a 0
              var s1=promediop;//asginacion sin re1
              double  calculo=s1*0.85;
            var ava=av*0.15;
            promedioca=calculo+ava;//calculo de promedio con avanzo
            }
          }
          if(promedioca<6){//si el promedio con la avanzo es menor que 6
            if(r2!="0" || r2!=0){//si la re2 es diferente de 0
              var s1=(promedioca+r2)/2;
              promediof=s1;  
            }else{// si la recuperacion es igual 0
                                      ...
                                      pruO = po.text;
                                        ...
                                        if (nie != "") {
                                          ...
                                          if (respuesta == "noEncontrado") {
                                            ...
                                            dynamic respuesta = await insertRe2(
